  ...
  return (
    <main className="main-content" style={{ background: '#050505' }}>
      <section style={{ paddingTop:'130px',paddingBottom:'50px',background:'linear-gradient(180deg,#050505 0%,#0a0a0a 100%)',borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ maxWidth:'800px',margin:'0 auto',padding:'0 24px' }}>
          <Link to={base} style={{ display:'inline-flex',alignItems:'center',gap:'6px',color:'#94A3B8',fontSize:'0.875rem',textDecoration:'none',marginBottom:'28px',transition:'color 0.2s' }}
            onMouseOver={e=>e.currentTarget.style.color='#d1bbfb'} onMouseOut={e=>e.currentTarget.style.color='#94A3B8'}>
            {t.blog.backToBlog}
          </Link>
          <div style={{ display:'inline-block',background:'rgba(209, 187, 251,0.12)',border:'1px solid rgba(209, 187, 251,0.3)',color:'#d1bbfb',padding:'4px 14px',borderRadius:'20px',fontSize:'0.75rem',fontWeight:700,letterSpacing:'0.1em',textTransform:'uppercase',marginBottom:'20px' }}>
            {post.date}
          </div>
          ...
          <div style={{ display:'flex',justifyContent:'space-between',gap:'16px',marginTop:'60px',paddingTop:'32px',borderTop:'1px solid rgba(255,255,255,0.08)',flexWrap:'wrap' }}>
            {prev ? (
              <Link to={`${base}/${prev.slug}`} style={{ flex:1,minWidth:'220px',background:'#0a0a0a',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'16px',padding:'20px',textDecoration:'none',transition:'all 0.2s',boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.05), inset 0 2px 10px rgba(255,255,255,0.1), 0 10px 30px rgba(0,0,0,0.5)' }}
                onMouseOver={e=>{e.currentTarget.style.borderColor='rgba(209, 187, 251,0.3)';e.currentTarget.style.transform='translateY(-2px)';e.currentTarget.style.boxShadow='inset 0 0 0 1px rgba(255,255,255,0.1), inset 0 2px 10px rgba(255,255,255,0.2), 0 20px 40px rgba(0,0,0,0.6)'}}
      ...
      <style>{`
        ...
        .blog-body a { color: #d1bbfb; text-decoration: underline; }
        .blog-body blockquote { border-left: 3px solid #d1bbfb; padding-left: 1.25em; margin: 1.5em 0; color: #94A3B8; font-style: italic; }
        .blog-body code { background: rgba(255,255,255,0.06); border-radius: 4px; padding: 2px 6px; font-size: 0.9em; color: #d1bbfb; }
        .blog-body table { width: 100%; border-collapse: collapse; margin: 1.5em 0; }
        .blog-body th { background: rgba(209, 187, 251,0.1); color: #F8FAFC; padding: 12px; text-align: left; border: 1px solid rgba(255,255,255,0.08); }
