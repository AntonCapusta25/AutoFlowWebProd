...
export default function ContactForm({ lang = 'en' }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!form.name || !form.email || !form.phone || !form.service) { showNotification(labels.errorFields,'error'); return }
    ...
    <form id="contactForm" onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:'20px'}}>
      ...
      <div className="form-group">
        ...
        <div className="input-wrapper">
          ...
          <input type="tel" required value={form.phone} onChange={e=>set('phone',e.target.value)} className="booking-input" />
        </div>
      </div>
      <div className="form-group">
